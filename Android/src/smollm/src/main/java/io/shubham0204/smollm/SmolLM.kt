...
class SmolLM {
    companion object {
        init {
            ...
            if (!isEmulated) {
                if (supportsArm64V8a()) {
                    // Try loading from most optimized to least, catching failures gracefully.
                    // We only ship: smollm_v8_2_fp16_dotprod, smollm_v8, smollm (universal)
                    val loaded = tryLoadLibrary("smollm_v8_2_fp16_dotprod", logTag)
                        || tryLoadLibrary("smollm_v8", logTag)
                        || tryLoadLibrary("smollm", logTag)
                    if (!loaded) {
                        Log.e(logTag, "Failed to load any arm64 native library!")
                    }
                } else if (Build.SUPPORTED_32_BIT_ABIS[0]?.equals("armeabi-v7a") == true) {
                    val loaded = tryLoadLibrary("smollm_v7a", logTag)
                        || tryLoadLibrary("smollm", logTag)
                    if (!loaded) {
                        Log.e(logTag, "Failed to load any armv7a native library!")
                    }
                } else {
        ...
        private fun tryLoadLibrary(name: String, logTag: String): Boolean {
            return try {
                Log.d(logTag, "Trying libsmollm variant: $name")
                System.loadLibrary(name)
                Log.d(logTag, "Loaded lib$name.so successfully")
                true
            } catch (e: UnsatisfiedLinkError) {
                Log.w(logTag, "lib$name.so not available: ${e.message}")
                false
            }
        }
    }
